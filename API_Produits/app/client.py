...
class APIProduit:
    def __init__(self, base_url, api_key=None, bearer_token=None):
        """
        Initialise un produit API avec l'URL de base et les informations d'authentification.
    ...
    def get(self, endpoint, params=None):
        return requests.get(
            self._make_url(endpoint), headers=self.headers, params=params, timeout=3000
        )
    ...
    def delete(self, endpoint):
        return requests.delete(
            self._make_url(endpoint), headers=self.headers, timeout=3000
        )
